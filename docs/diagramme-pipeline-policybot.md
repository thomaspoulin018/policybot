...
`CriterionFinding` est la source de vérité d'une ligne du rapport.
`InterviewState` est le seul objet qui traverse l'orchestrateur jusqu'au rendu.
...
classDiagram
  ...
  class ToolRef {
    ...
    +str vendor
    +IagType iag_type
    +str version_plan_tarifaire
    +list~CriterionFinding~ findings
    +ContractOfferingIdentity offering
  ...
  class CriterionFinding {
    ...
    +str question
    +str answer
    +RiskLetter inherent_risk
    +str justification
    +list~CriterionCitation~ citations
    +int rejected_citations
    +str exa_type
    +float cost_dollars
    +str outcome
  }
  class CriterionCitation {
    +str url
    +str title
    +str text
    +int begin
  ...
  class ContractOfferingIdentity {
    +str vendor
    +str product
    +str plan
    +str deployment_mode
    ...
    +str jurisdiction
    +date effective_date
    +missing_search_identity_fields() tuple
    +display_label() str
  }
  class Usage {
    +str description
    ...
    +bool rens_personnels
    +bool efvpr_required
    +list~str~ mode
    +str frequence_utilisation
    +int nb_utilisateurs
    +list~str~ result_use
    +bool automated_decisions
    +float classifier_confidence
    +bool needs_officer_confirmation
  }
  class RequestInfo {
    +str numero
    +str demandeur
  ...
  InterviewState "1" --> "0..*" ToolRef
  InterviewState "1" --> "0..*" Usage
  InterviewState "1" --> "1" RequestInfo
  ToolRef "1" --> "0..17" CriterionFinding
  ToolRef "1" --> "0..1" ContractOfferingIdentity
  CriterionFinding "1" --> "0..3" CriterionCitation
```

## 7. Dépendances entre paquets

Aucun cycle. `models`, `config`, `tracing`, `contract/criteres` et
`report/docx_tools` ne dépendent d'aucun autre module PolicyBot. `cli`
n'importe `interview/` et `report/` qu'après la barrière `--dry-run`, et
`interview/orchestrator` n'importe `contract/exa` qu'au moment de chercher.

```mermaid
flowchart LR
  CLI["cli"] --> INTAKE["intake/"]
  CLI --> CONFIG["config"]
  CLI -.->|"import différé, hors --dry-run"| FACTORY["interview/factory"]
  CLI -.->|"import différé, hors --dry-run"| GRILLE["report/grille"]
  CLI -.->|"import différé, hors --dry-run"| RENDER["report/renderer"]

  INTAKE --> MODELS["models"]
  INTAKE --> OFFER["contract/offering"]
  INTAKE --> REGISTRY["classify/tool_registry"]

  FACTORY --> CONFIG
  FACTORY --> LLM["llm"]
  FACTORY --> ORCH["interview/orchestrator"]

  ORCH --> CLASSD["classify/data_classifier"]
  ORCH --> REGISTRY
  ORCH --> CRITERES["contract/criteres"]
  ORCH --> OFFER
  ORCH --> LLM
  ORCH --> MODELS
  ORCH --> TRACING["tracing"]
  ORCH -.->|"import différé, au moment de chercher"| EXA["contract/exa"]

  EXA --> CRITERES
  EXA --> CITATIONS["contract/citations"]
  EXA --> SOURCE["contract/source_policy"]
  EXA --> REGISTRY
  EXA --> CONFIG
  EXA --> MODELS
  EXA --> TRACING
  ...
  GRILLE --> CRITERES
  GRILLE --> RENDER
  GRILLE --> DOCXT["report/docx_tools"]
  GRILLE --> MODELS
  RENDER --> MODELS

  classDef leaf fill:#f0fdf4,stroke:#22c55e,color:#166534;
  class MODELS,CONFIG,TRACING,CRITERES,DOCXT leaf;
```

## Documents liés

- [`diagramme-recherche-critere.md`](diagramme-recherche-critere.md) — le cycle
  de vie d'une des dix-sept recherches, jusqu'à la citation ancrée.
- [`diagramme-cache-arp.md`](diagramme-cache-arp.md) — **obsolète** : le cache
  ARP décrit n'existe plus dans le code.
- [`carte-pipeline-policybot.html`](carte-pipeline-policybot.html) — table de
  correspondance nœud → fichier ; rédigée avant le retrait du cache.
</content>
</invoke>
